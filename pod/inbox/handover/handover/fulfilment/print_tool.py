...
# Every store's bucket, tried in turn. A custom label does not say which
# store it came from, so the tool has to look in all of them.
#
# This is why m12k orders failed: only store 1's bucket was listed, and
# m12k designs live in a different bucket entirely.
BUCKETS = [
...
# m12k was generated with --jpeg, so its designs are .jpg while store 1's
# are .png. Try both.
EXTS = ["png", "jpg"]
DPI = 300
CANVAS = (3600, 4800)
...
def fetch(did):
    """Find a design in whichever store it belongs to.

    A custom label carries no store, so try every bucket and both file
    extensions. Store 1 wrote .png; m12k was generated with --jpeg and
    wrote .jpg. Looking in one bucket for one extension is why m12k
    orders came back "not recognised".
    """
    last = None
    for base in BUCKETS:
        for ext in EXTS:
...
class App:
    ...
    def run(self, skus, width):
        ...
        for i, sku in enumerate(skus, 1):
            m = re.search(r"(\d+)", sku)
            if not m:
                self.say(f"  [{i}] {sku} — no design number in this label")
                self.bar["value"] = i
                continue
            did = str(int(m.group(1)))
            try:
                art = to_print(fetch(did), width, solid=self.solid.get(),
                ...
                mreq = urllib.request.Request(
                    f"{BASE}/art/mock/{did}.jpg", headers={"User-Agent": UA})
                for ctx in CTXS:
                    try:
                ...
                self.say(f"  [{i}] {sku} — done{tif_note}{warn}")
                ok += 1
            except Exception as e:
                msg = str(e)
                if "404" in msg:
