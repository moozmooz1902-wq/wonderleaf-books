...
# Bucket lookup lives in one shared module so order.py and print_tool.py
# cannot drift apart, and so the store list comes from R2 rather than
# from a list edited by hand on every Mac.
import wl_lookup
...
ap.add_argument("--stores", action="store_true",
                help="list the stores this tool knows about, then exit")
...
def locate(label):
    """
    Find this order's design in whichever store it belongs to.

    The label is used exactly as eBay gave it. The previous version pulled
    the first run of digits out of the label, which silently fetched the
    wrong artwork for any label that is not purely numeric - every design in
    the current catalogue, e.g. bd_16_Dad_0 became 16.
    """
    return wl_lookup.find(label, extra_base=args.base)
...
for i, sku in enumerate(skus, 1):
    d = locate(sku)
    if d is None:
        print(f"  [{i}] {sku}: not found in any store "
    ...
    try:
        ...
        if wl_lookup.print_ready(im):
            # Already a finished 4500x5400 transparent print file. Putting it
            # through to_print() would flatten it onto black and rebuild the
            # alpha from luminance, which softens type that is currently
            # crisp. Ship it as it is.
            art = im
        else:
            art = to_print(im.convert("RGB"))
        ...
        # listing uses. It is fetched from the bucket the DESIGN was found in.
        # Fetching it from the first bucket regardless is why store 2 orders
        # never got a real shirt image and always fell back to the flattened
        # version below.
        ...
        warn = (f"   ** {flat*100:.0f}% flat grey — check for a slab artefact"
                if flat > 0.04 else "")
        print(f"  [{i}] {sku} -> {dst}   [{d.store or d.base}]{warn}")
        ok += 1
