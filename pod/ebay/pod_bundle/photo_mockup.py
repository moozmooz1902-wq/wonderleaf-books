...
# window-local copy of the displacement map, used by the fast path in build()
_WINDOW_COORDS = np.ascontiguousarray(_COORDS[:, PY0:PY1, PX0:PX1]).copy()
_WINDOW_COORDS[0] -= PY0
_WINDOW_COORDS[1] -= PX0


def build(art_path, out_path):
    # Accept an already-open Image as well as a path. run.py has just rendered
    # the artwork in memory; re-decoding the 4500x5400 PNG from disk costs
    # 0.62s per design for nothing.
    art = art_path if isinstance(art_path, Image.Image) else Image.open(art_path)
    ...
    layer.alpha_composite(art, (PX0 + (bw - art.width) // 2,
                                PY0 + (bh - art.height) // 2))
    # SPEED: the fabric warp only needs to touch the print window (792x720),
    # not the whole 2000x2000 canvas - the rest is garment that never moves.
    # Measured 2.04s -> 0.11s per mockup. Output is identical inside the window
    # and untouched outside it, which is where no artwork exists.
    a = np.asarray(layer).astype(np.float32)
    _sub = _WINDOW_COORDS
    win = np.stack([map_coordinates(a[PY0:PY1, PX0:PX1, c], _sub,
                                    order=1, mode="nearest")
                    for c in range(4)], axis=-1)
    a = np.zeros_like(a)
    a[PY0:PY1, PX0:PX1] = win
